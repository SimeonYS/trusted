...
class TrustedPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('trusted.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `trusted`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM trusted WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO trusted (date, title, link, content)"
